...
const LeaderboardPage = () => {
  ...
  useEffect(() => {
    const fetchLeaderboard = async () => {
      try {
        const response = await fetch("https://code-clash-nza1.onrender.com/api/leaderboard");
          ...
          >
            ...
            <div className="divide-y divide-gray-700/50">
              {leaderboard.map((entry) => (
                ...
                >
                  <div
                    className={`w-8 h-8 flex items-center justify-center rounded-full mr-3 ${entry.rank === 1
            ...
            <div className="bg-gray-900/30 p-3 border-t border-gray-700/50 text-center">
              <p className="text-xs text-gray-400">Play today's quiz to join the leaderboard!</p>
            </div>
          </motion.div>
          ...
          >
            <div className="bg-gray-800/50 backdrop-blur-md rounded-xl border border-gray-700/50 p-6">
              ...
              <p className="text-3xl font-bold text-blue-400">
                {leaderboard.find((entry) => entry.isYou)?.rank || "Unranked"}
              </p>
              <p className="text-xs text-gray-500 mt-1">Top 5% of players</p>
            </div>
            <div className="bg-gray-800/50 backdrop-blur-md rounded-xl border border-gray-700/50 p-6">
              <h4 className="text-sm font-medium text-gray appalled-gray-400 mb-2">Your Score</h4>
              <p className="text-3xl font-bold text-purple-400">
                {leaderboard.find((entry) => entry.isYou)?.score || 0}
              </p>
